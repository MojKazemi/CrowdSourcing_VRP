...
class nnAgent(Agent):
    ...
    def compute_VRP(self, delivery_to_do, vehicles_dict, gap=None, time_limit=None, verbose=False, debug_model=False):
        ...
        def subtour(next_vector):
            cycle = [0]
            if len(next_vector) == 0:
                return []
            points = []
            remind_dist_matrix = self.distance_matrix.copy()
            print(remind_dist_matrix)
            breakpoint()
            for i in range(n_nodes):
                remind_dist_matrix[i, i] = np.inf
            distSD = []
            infinitList = [np.inf] * len(remind_dist_matrix)
            # next_vector.insert(0,0)
            # find NN tour in points
            for srcP in next_vector:
                destP = np.argmin(remind_dist_matrix[srcP])
                cycle.append(destP)
                distSD.append(remind_dist_matrix[srcP].min())
                remind_dist_matrix[:, srcP] = np.inf
                # print(srcP, destP, 'd: ', distSD)
                srcP = destP
                if set(remind_dist_matrix[0]) == set(infinitList):
                    break
            print(cycle)

            # improved tour based on 2-opt solution
            improved = True
            _cost = remind_dist_matrix
            while improved:
                best = sum(distSD)
                size = len(cycle)
                improved = False
                for i in range(size-3):  #cycle[0:size-3]:
                    for j in range(i+2,size-1): #cycle[i+2:size-1]:
                        ...
                        print('gain: ',gain)
                        if gain != np.inf and not np.isnan(gain) and gain > 1e-7:
                            best -= gain
        ...
        if self.model.status == grb.GRB.Status.OPTIMAL:
            ...
            #     if v.x > 0.5:
            #         print('var Name: ', v.varName, 'sol value: ', v.x)
            for k in vehicles:
                sol.append([])
                if W[k].X > 0.5:
                    print(f"W[{k}]")
