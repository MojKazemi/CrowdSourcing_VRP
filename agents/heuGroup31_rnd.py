...
class heuGroup31_rnd(Agent):

    def __init__(self, env):
        self.env = env
        self.name = 'heuGroup31_RND'
        self.quantile = 0.5
        self.quantile_crowd = 0.5
        self.numCrowd = 10
        ...
        # dist_class = distFromFile('./cfg/distance_matrix_sample.csv')
        dist_class = distFromFile('./cfg/distance_matrix.csv')
        # dist_class = distFromFile('./cfg/dist_matrix_rnd.csv')
        self.distance_matrix = dist_class.distMatrix()
    ...
    def compute_delivery_to_crowdship_dist(self, deliveries):
        if len(deliveries) == 0:
            return []
        points = []
        self.delivery = []
        ...
        # dist_class = distFromFile('./Real Instance/distance_matrix.csv')
        # distance_matrix = dist_class.distMatrix()
    ...
    def compute_delivery_to_crowdship(self, deliveries):
        ...
        # for i in range(self.numCrowd):
        #     dupflag = True
        #     while dupflag:
        #         _index = round(np.random.uniform()*len(deliveries))
        #         print('_index: ',_index)
        #         print('indexlist: ',indexlist)
        #         if indexlist.count(_index) == 0:
        #             indexlist.append(_index)
        #             id_to_crowdship.append(_index)
        #             dupflag = False
    ...
    def compute_VRP(self, delivery_to_do, vehicles_dict):
        ...
        # dist_class = distFromFile('./Real Instance/distance_matrix.csv')
        # dist_dict = dist_class.exportDist()
    ...
    def learn_and_save(self):
        ...
        self.data_improving_quantile['numCrowd'].append(self.numCrowd)
    ...
    def start_test(self):
        ...
        db.to_csv('./results/data_improving_rnd_500.csv')
        pos_min = self.data_improving_quantile['performance'].index(min(self.data_improving_quantile['performance']))
        self.numCrowd = self.data_improving_quantile['numCrowd'][pos_min]
